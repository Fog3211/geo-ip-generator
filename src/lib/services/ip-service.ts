...
export async function generateIpByCountry(input: z.infer<typeof generateIpSchema>) {
  ...
  return await withCache(
    ...
    async () => {
      ...
      const startOffset = Math.floor(Math.random() * totalRangesCount);
      const selectedRanges = await db.ipRange.findMany({
        where: { countryId: country.id },
        select: rangeSelect,
        orderBy: { id: 'asc' },
        skip: startOffset,
      ...
      if (selectedRanges.length < count) {
        const remaining = count - selectedRanges.length;
        const wrappedRanges = await db.ipRange.findMany({
          where: { countryId: country.id },
          select: rangeSelect,
          orderBy: { id: 'asc' },
          take: remaining,
        });
        selectedRanges.push(...wrappedRanges);
      }

      if (selectedRanges.length === 0) {
        throw new Error('Failed to fetch IP ranges. Please retry.');
      }

      const generatedIps = selectedRanges.map((chosen) => ({
        ip: generateRandomIpInRange(chosen.startIp, chosen.endIp),
        location: {
